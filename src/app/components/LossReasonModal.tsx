  ...
  return (
    <>
        ...
        footer={mode !== 'view' ? (
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2">
              {hasUnsavedChanges && (
                <span className="flex items-center gap-1.5 text-caption text-amber-600 bg-amber-50 px-3 py-1.5 rounded-full">
      ...
      >
        ...
        {/* Trade Summary Bar */}
        {tradeData && (
          <div className="bg-gradient-to-r from-slate-50 to-violet-50/50 border border-slate-200 rounded-xl p-4 mb-6">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-3">
                <span className="text-card-title font-bold text-slate-900">{tradeData.pair}</span>
                <span className={`px-2.5 py-0.5 rounded-md text-table-header font-bold uppercase ${
              ...
              {displayIsValid !== undefined && (
                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-caption ${
        ...
        {!tradeData && mode === 'view' && existingAnalysis && (
          <div className="bg-gradient-to-r from-slate-50 to-violet-50/50 border border-slate-200 rounded-xl p-4 mb-6">
            <div className="flex items-center gap-2 text-body text-slate-600">
              ...
              <span className={`ml-auto inline-flex items-center px-2.5 py-1 rounded-full text-caption ${
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <select
                ...
                disabled={mode === 'view'}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#7C3AED]/30 focus:border-[#7C3AED] disabled:opacity-60 disabled:cursor-not-allowed"
              >
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <label className="block text-body-sm text-slate-700 mb-3">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            {mode === 'view' && description ? (
              <div
                className="p-4 bg-slate-50 rounded-xl text-body text-slate-700 whitespace-pre-wrap"
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#7C3AED]/30 focus:border-[#7C3AED] resize-none disabled:opacity-60 disabled:cursor-not-allowed"
              />
            )}
          </CardContent>
        </Card>
        ...
        <Card className="mb-2">
          ...
          <CardContent>
                ...
                return (
                  <div key={tf}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-table-header font-bold text-slate-600 uppercase">{tf} Chart</span>
                      {img && (
                        <span className="text-caption text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">Uploaded</span>
                      )}
                    </div>
                    <div className="relative group">
                      <div className={`
                        ...
                        ${img
                          ? 'border-slate-200 shadow-sm hover:shadow-lg hover:border-[#7C3AED]'
                          : 'border-dashed border-slate-300 bg-slate-50 hover:bg-slate-100 hover:border-[#7C3AED]'
                        }
                      `}>
                        {img ? (
                          <>
                            ...
                            <span className="absolute top-2 left-2 px-2 py-1 bg-slate-900/70 text-white text-caption font-bold rounded-md backdrop-blur-sm">
                              {tf}
                            </span>
                            {mode !== 'view' && (
                              <button
                          ...
                          <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer">
                            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-violet-100 to-purple-100 flex items-center justify-center mb-2 shadow-inner">
                              <Upload className="w-6 h-6 text-[#7C3AED]" />
                            </div>
                            <span className="text-body-sm text-slate-600">Upload {tf} Chart</span>
                            <span className="text-caption text-slate-400 mt-1">PNG, JPG up to 10MB</span>
                            <input
                              ref={fileInputRef}
                              type="file"
                              accept="image/*"
                          ...
                          <div className="flex flex-col items-center justify-center w-full h-full text-slate-400">
                            ...
                            <span className="text-body-sm">No chart uploaded</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {uploading && (
              <div className="flex items-center justify-center gap-2 mt-4 p-3 bg-violet-50 border border-violet-200 rounded-xl text-body text-[#7C3AED]">
                <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
        ...
        {mode === 'view' && tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {tags.map((tag, i) => (
              <span key={i} className="px-3 py-1 bg-slate-100 text-slate-600 text-caption rounded-full">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowUnsavedConfirm(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-card-title font-bold text-slate-900">Unsaved Changes</h3>
                <p className="text-body text-slate-500">You have unsaved changes that will be lost.</p>
              </div>
            </div>
        ...
        >
          ...
          >
            <div className="relative">
              ...
              />
              <span className="absolute bottom-4 left-4 px-3 py-1.5 bg-slate-900/80 text-white text-body-sm font-bold rounded-md backdrop-blur-sm">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/40 text-caption">
