...
export default function WeeklyReviewList() {
  ...
  const [filters, setFilters] = useState({ pair: '', week: '', year: '', bias: '', search: '' });
  ...
  const loadReviews = async () => {
    ...
    setError(null);
    try {
      const result = await apiService.weeklyReviews.getAll({
        pair: filters.pair || undefined,
        weekNumber: filters.week ? parseInt(filters.week) : undefined,
  ...
  const handleOpen = (review: any) => {
    (window as any).__weeklyReviewId = review.id || review._id;
    window.dispatchEvent(new CustomEvent('navigate-to-tab', { detail: 'weekly-review-detail' }));
  };

  const handleEdit = (review: any) => {
    (window as any).__weeklyReviewEditId = review.id || review._id;
    window.dispatchEvent(new CustomEvent('navigate-to-tab', { detail: 'weekly-review-form' }));
  };

  const handleDelete = async (review: any) => {
    if (!confirm(`Delete review for ${review.pair}?`)) return;
    try {
      await apiService.weeklyReviews.delete(review._id);
      loadReviews();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-page-title text-[#0F172A] font-semibold">Weekly Market Review</h1>
          <p className="text-body text-[#64748B] mt-1">Your weekly trading analysis and key level planning.</p>
        </div>
        <Button onClick={handleCreateNew} className="bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-lg shadow-blue-600/25 hover:shadow-xl hover:shadow-blue-600/30 hover:-translate-y-0.5">
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <Select value={filters.week} onValueChange={(v) => handleFilterChange('week', v)}>
          <SelectTrigger className="w-full sm:w-36">
            <SelectValue placeholder="Any week" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="Any week" disabled className="hidden">Any week</SelectItem>
            {WEEKS.map(w => (
      ...
      {!error && !isLoading && reviews.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-body text-slate-400 mb-6">Create your first weekly market review to start tracking weekly analysis.</p>
