    ...
    <div className="max-w-7xl mx-auto space-y-6">
          ...
          {TIMEFRAMES.map(tf => {
            ...
            return (
              <CardContainer key={tf}>
                <div className={cn('px-3 py-1.5 rounded-full text-xs font-medium inline-block mb-3', TIMEFRAME_COLORS[tf])}>
                  {TIMEFRAME_LABELS[tf]}
                </div>
                {entry ? (
                  <div className="space-y-2">
                    <div className={cn('px-2 py-1 rounded text-xs font-medium text-center', DIRECTION_COLORS[entry.crtDirection] || 'bg-slate-100 text-slate-600')}>
                      {entry.crtDirection}
                    </div>
                    <div className="text-xs text-slate-600">
                      <span className="font-medium">Key Level:</span> {getKeyLevelDisplay(entry)}
                    </div>
                    <div className={cn('px-2 py-1 rounded text-xs font-medium text-center border', STATUS_COLORS[entry.crtStatus] || 'bg-slate-100 text-slate-500')}>
                      {entry.crtStatus}
                    </div>
                    ...
                    {entry.imagePath && (
                      <img src={entry.imagePath} alt="CRT" className="w-full h-16 object-cover rounded border border-slate-200 cursor-pointer"
      ...
      <CardContainer>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-3">
            ...
            <div>
              ...
              <div className="relative">
                <input type="text" value={filters.search} placeholder="Notes, key level..." onChange={e => setFilters(p => ({ ...p, search: e.target.value }))} className="w-full px-2 py-1.5 pr-8 border border-slate-200 rounded-lg text-sm" />
        ...
        <div className="flex gap-2">
          <button onClick={handleFilter} className="flex items-center gap-1 px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600"><Filter className="w-4 h-4" /> Apply</button>
          <button onClick={loadEvents} className="flex items-center gap-1 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm hover:bg-slate-200"><Calendar className="w-4 h-4" /> Refresh</button>
        </div>
      </CardContainer>
            ...
            <>
                    ...
                    {events.map((event, idx) => (
                      <tr key={event.id || idx} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-3 px-3">
                          <span className={cn('px-2 py-0.5 rounded text-xs font-medium', TIMEFRAME_COLORS[event.timeframe])}>{TIMEFRAME_LABELS[event.timeframe] || event.timeframe}</span>
                        </td>
                        <td className="py-3 px-3 text-sm text-slate-700">
                          {event.date?.split('T')[0]}{event.time && <span className="text-xs text-slate-400 ml-1">{event.time}</span>}
                        </td>
                        <td className="py-3 px-3">
                          <span className={cn('px-2 py-0.5 rounded text-xs font-medium inline-block', DIRECTION_COLORS[event.crtDirection] || 'bg-slate-100 text-slate-600')}>{event.crtDirection}</span>
                        </td>
                        <td className="py-3 px-3 text-sm text-slate-700 max-w-[120px] truncate" title={getKeyLevelDisplay(event)}>{getKeyLevelDisplay(event)}</td>
                        <td className="py-3 px-3">
                          <span className={cn('px-2 py-0.5 rounded text-xs font-medium inline-block border', STATUS_COLORS[event.crtStatus] || 'bg-slate-100 text-slate-500')}>{event.crtStatus}</span>
                        </td>
                        <td className="py-3 px-3 text-sm text-slate-700">{event.crtRangeRespected}</td>
                        <td className="py-3 px-3">
                          {event.imagePath ? (
                            <img src={event.imagePath} alt="CRT" className="w-10 h-10 object-cover rounded border border-slate-200 cursor-pointer hover:opacity-80 transition-opacity"
                              onClick={() => setViewingImage([{ url: event.imagePath, label: `${TIMEFRAME_LABELS[event.timeframe]} CRT` }])} />
                          ) : <span className="text-xs text-slate-400">--</span>}
                        </td>
                        <td className="py-3 px-3 text-sm text-slate-500 max-w-[150px] truncate" title={event.notes}>{event.notes || '--'}</td>
                        <td className="py-3 px-3 text-xs text-slate-400">{format(new Date(event.createdAt), 'MMM dd, yyyy')}</td>
                        <td className="py-3 px-3">
                          <div className="flex items-center gap-1">
                            <button onClick={() => setEditingEvent(event)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Edit"><Edit className="w-4 h-4" /></button>
                            <button onClick={() => setViewingEvent(event)} className="p-1.5 text-slate-600 hover:bg-slate-100 rounded transition-colors" title="View"><Eye className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              {pagination.pages > 1 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} className="p-1.5 border border-slate-200 rounded hover:bg-slate-50 disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button>
                    <button onClick={() => setPage(p => Math.min(pagination.pages, p + 1))} disabled={page >= pagination.pages} className="p-1.5 border border-slate-200 rounded hover:bg-slate-50 disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
            </>
      ...
      <Dialog.Root open={!!editingEvent} onOpenChange={(open) => !open && setEditingEvent(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-xl shadow-2xl border border-slate-200">
      ...
      <Dialog.Root open={!!viewingEvent} onOpenChange={(open) => !open && setViewingEvent(null)}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-xl shadow-2xl border border-slate-200">
  ...
  return (
    <div>
      <div className="flex items-center justify-between p-5 border-b border-slate-200">
        <div className="flex items-center gap-2">
          <span className={cn('px-3 py-1 rounded-full text-sm font-medium', TIMEFRAME_COLORS[tf])}>{TIMEFRAME_LABELS[tf] || tf}</span>
          <span className="text-sm font-semibold text-slate-700">{event.pair}</span>
        </div>
      ...
      <div className="p-5 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" value={event.date?.split('T')[0]} disabled className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Time</label>
            <input type="time" value={event.time} disabled className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-500" />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Strategy comments, observations..." rows={3} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm min-h-[72px]" />
        </div>
      </div>

      <div className="flex items-center justify-end gap-3 p-5 border-t border-slate-200">
        <button onClick={onClose} className="px-4 py-2 text-sm text-slate-600 hover:text-slate-800">Cancel</button>
        <button onClick={handleSubmit} disabled={isSaving} className="flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 disabled:opacity-50">
    ...
    <div>
      ...
      <div className="p-5 space-y-4">
        ...
        {event.statusHistory && event.statusHistory.length > 0 && (
          <div>
            <label className="block text-xs text-slate-500 mb-1">Status History</label>
            <div className="space-y-1">
              {event.statusHistory.map((h, i) => (
                <div key={i} className="flex items-center justify-between text-xs text-slate-600 bg-slate-50 px-3 py-1.5 rounded">
                  <span className={cn('px-2 py-0.5 rounded text-xs font-medium border', STATUS_COLORS[h.status] || 'bg-slate-100 text-slate-500')}>{h.status}</span>
                  <span>{format(new Date(h.date), 'MMM dd, yyyy HH:mm')}</span>
                </div>
              ))}
            </div>
          </div>
...
function Field({ label, value, badge }: { label: string; value: string; badge?: string }) {
  return (
    <div>
      ...
      {badge ? (
        <span className={cn('px-2 py-0.5 rounded text-xs font-medium inline-block', badge)}>{value}</span>
      ) : (
