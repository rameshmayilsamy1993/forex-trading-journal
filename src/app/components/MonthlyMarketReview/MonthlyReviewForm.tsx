...
const inputClass = "h-12 rounded-[14px] border border-[#E2E8F0] bg-white px-4 text-[15px] font-medium text-[#0F172A] placeholder:text-[#94A3B8] outline-none focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/20 w-full";
const textareaClass = "rounded-[14px] border border-[#E2E8F0] bg-white px-4 py-3 text-[15px] font-medium text-[#0F172A] placeholder:text-[#94A3B8] outline-none focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/20 w-full resize-none";
const selectTriggerClass = "h-12 rounded-[14px] border border-[#E2E8F0] bg-white px-4 text-[15px] font-medium text-[#0F172A] outline-none focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/20 w-full";
  ...
  useEffect(() => {
    ...
    apiService.monthlyReviews.getById(reviewId)
      .then((review) => {
        ...
        setBias(review.bias || '');
        setCrtDirection(review.monthlyCrt || '');
        setNarrative(review.summary || '');
        return apiService.monthlyReviews.getEntries(reviewId).catch(() => []);
      })
      .then(entries => {
        if (entries.length > 0) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (allImages.length > 0) {
        await apiService.monthlyReviews.createEntry(savedId, {
      ...
      (window as any).__monthlyReviewId = savedId;
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8 space-y-8">
      <nav className="flex items-center gap-2 text-[13px] font-medium text-[#64748B]">
        <button type="button" onClick={handleBack} className="hover:text-[#7C3AED]">Monthly Market Review</button>
        <ChevronRight className="size-3.5 text-[#CBD5E1]" />
        <span className="text-[#0F172A]">{isEditMode ? 'Edit' : 'New'} Monthly Review</span>
      </nav>

      <div>
        <h1 className="text-[28px] font-bold text-[#0F172A]">{isEditMode ? 'Edit Monthly Review' : 'Monthly Market Review'}</h1>
        <p className="text-[15px] font-medium text-[#64748B] mt-1">Set your monthly bias, CRT, and add screenshots.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E5EAF2] p-8 space-y-6">
        ...
        <div className="space-y-2">
          <Label className={labelClass}>Monthly Bias</Label>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder="e.g., Bullish CRT on Monthly"
        ...
        <div className="space-y-2">
          ...
          {newsScreenshot ? (
            ...
            <label className="flex items-center justify-center h-20 border-2 border-dashed border-[#E2E8F0] rounded-xl cursor-pointer hover:border-[#7C3AED] hover:bg-purple-50">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="Your monthly market observations and key levels..."
            rows={5}
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <Button type="button" onClick={handleSubmit} disabled={isSaving}
          className="bg-gradient-to-r from-[#7C3AED] to-[#6D28D9] text-white shadow-lg shadow-[#7C3AED]/25 px-8">
